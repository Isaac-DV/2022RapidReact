...
/** Add your docs here. */
public class TwoBallCloseHideMode extends AutoModeBase {
    Superstructure s;
    @Override
    public List<Trajectory<TimedState<Pose2dWithCurvature>>> getPaths(){
        return Arrays.asList(trajectories.thirdBallBackup, trajectories.thirdBallToThirdOpponentBall,
            trajectories.thirdOpponentBallToCloseEject, trajectories.closeEjectToMidline
        );
    }
    
    public TwoBallCloseHideMode() {
    ...
    protected void routine() throws AutoModeEndedException {
        ...
        if(s.ballFeeder.hasIntakeResuckBeenActivated()) {
            runAction(new SetTrajectoryAction(trajectories.thirdOpponentBallToCloseEject, -135.0, 0.5));
            s.wrist.setWristAngle(80.0);
            runAction(new WaitToFinishPathAction(7.0));
            s.intake.setOpenLoop(-0.15);

            runAction(new WaitAction(1.0));
            runAction(new SetTrajectoryAction(trajectories.closeEjectToMidline, 0, 1.0));
            runAction(new WaitToPassXCoordinateAction(108.0));
            s.intakeState();
            runAction(new WaitForSuperstructureAction());
            s.visionShotState();
